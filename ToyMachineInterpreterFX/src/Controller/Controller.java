...
public class Controller {
    ...
    public int noOfProgramStates() {
        return repository.getProgramList().size();
    }

    public ProgramState getProgramStateByIndex(int index) {
        return repository.getProgramList().get(index);
    }

    public ObservableList<String> getProgramStatesID() {
        ObservableList<String> list = FXCollections.observableArrayList();
        for (ProgramState state : repository.getProgramList()) {
            list.add(String.valueOf(state.getId()));
        }
        return list;
    }
    ...
    public boolean oneStepGUI() {
        ...
        if (programs.size() > 0) {
            ...
            removeCompletedPrograms(repository.getProgramList());
            return true;
        } else {
            executor.shutdownNow();
